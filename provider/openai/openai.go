...
// It returns a descriptive error on non-2xx, including the response body.
func doJSON(ctx context.Context, hc *http.Client, baseURL, apiKey, path string, body, out any) error {
	buf, err := json.Marshal(body)
	if err != nil {
		return err
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, trimBase(baseURL)+path, bytes.NewReader(buf))
	if err != nil {
		return err
	}
